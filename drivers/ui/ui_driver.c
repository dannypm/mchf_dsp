...
void ui_driver_thread(void)
{
	...
	// -- change audio volume
	#if 0
	if(ts.audio_gain != ts.audio_gain_req)
	{
		// Set public
		ts.audio_gain = ts.audio_gain_req;
	...
	{
		// Public set inside the function
		// --
	...
	//
	// change filter
	if(ts.filter_id != ts.api_filter_id)
	{
		// Set public
		ts.filter_id = ts.api_filter_id;
	...
	//
	// change tuning step
	/*if(df.tuning_step != df.tuning_step_req)
	{
		// Set public
		df.tuning_step = df.tuning_step_req;
...
void UiDrawSpectrumScopeFrequencyBarText(void)
{
	...
	if(!sd.magnify)
	{		// if magnify is off, way *may* have the graticule shifted.  (If it is on, it is NEVER shifted from center.)
#ifndef DSP_MODE
...
skip_check:
	...
	if(mode != 3)	{		// updating ONLY the TX frequency display?
		...
		if(ui_si570_set_frequency(ts.tune_freq,ts.freq_cal,df.temp_factor, 1))
		{	// did the tuning require that a large tuning step occur?

			if(ts.sysclock > RX_MUTE_START_DELAY)
			{	// has system start-up completed?
